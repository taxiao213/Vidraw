const createFFmpegCore = (function () {
  let _scriptDir =
  ...
  return function (createFFmpegCore) {
    ...
    pa = !ma && !l && !oa;
    const n = f.ENVIRONMENT_IS_PTHREAD || !1;
    n && (qa = f.buffer);
    let ra = "";
    function sa(a) {
    ...
    var h = f.printErr || console.warn.bind(console);
    for (ia in fa) {
    ...
    let F;
    var Ua;
    let Va = f.INITIAL_MEMORY || 1073741824;
    if (n) {
      (Ca = f.wasmMemory), (qa = f.buffer);
    ...
    var L = {
      ...
      Pg(a) {
        if (a) {
          if (a.threadInfoStruct) {
            let b = D[(a.threadInfoStruct + 104) >> 2];
      ...
      Vh() {
        let a = sa("ffmpeg-core.worker.js");
    ...
    function Kb() {
      ...
      if (!Kb.Yh) {
        ...
        let d = new Date(b, 6, 1);
        b = c.getTimezoneOffset();
        const e = d.getTimezoneOffset();
        let g = Math.max(b, e);
        D[Lb() >> 2] = 60 * g;
        D[Mb() >> 2] = Number(b != e);
        c = a(c);
    ...
    function Ob(a, b) {
      ...
      D[(b + 24) >> 2] = a.getDay();
      let c = new Date(a.getFullYear(), 0, 1);
      D[(b + 28) >> 2] = ((a.getTime() - c.getTime()) / 864e5) | 0;
      D[(b + 36) >> 2] = -(60 * a.getTimezoneOffset());
      const d = new Date(a.getFullYear(), 6, 1).getTimezoneOffset();
    ...
    var N = {
      ...
      open(a, b, c, d, e) {
        ...
        !f.logReadFiles ||
          ...
          (N.$g || (N.$g = {}),
          a in N.$g ||
            ((N.$g[a] = 1), h("FS.trackingDelegate error on read file: " + a)));
        try {
          N.mf.onOpenFile &&
            ((e = 0),
        ...
        g.prototype.kh = function () {
          ...
          x.Wh(function (z) {
            ...
            if ("undefined" === typeof x.Sf[z]) {
              ...
              if (I > V)
                throw Error(
                  "invalid range (" + I + ", " + V + ") or no bytes requested!",
                );
              if (V > t - 1)
                throw Error(
    ...
    var Q = {
      ...
      qf: {
        ...
        Zf(a) {
          ...
          let b = 0;
            var c = 1 === a.type ? Q.qf.vg(a, a.sf, a.vf) : null;
          if (
            a.ag.length ||
            !c ||
        ...
        close(a) {
          ...
          for (let b = Object.keys(a.ng), c = 0; c < b.length; c++) {
            let d = a.ng[b[c]];
            try {
              d.socket.close();
            } catch (e) {}
        ...
        listen(a) {
          ...
          let b = require("ws").Server;
          a.lf = new b({ host: a.Bg, port: a.Qf });
          f.websocket.emit("listen", a.stream.fd);
          a.lf.on("connection", (c) => {
            if (1 === a.type) {
              let d = Q.createSocket(a.family, a.type, a.protocol);
              c = Q.qf.tg(d, c);
              d.sf = c.hf;
              d.vf = c.port;
              a.pending.push(d);
        ...
        bh(a, b) {
          ...
          let d = c.data.byteLength || c.data.length;
            var e = c.data.byteOffset || 0;
            var g = c.data.buffer || c.data;
          b = Math.min(b, d);
          let k = { buffer: new Uint8Array(g, e, b), hf: c.hf, port: c.port };
          1 === a.type &&
            b < d &&
    ...
    function Wc(a, b, c) {
      ...
      if (ma) {
        ...
        for (Atomics.exchange(D, L.Vf >> 2, a); ; ) {
          d = performance.now();
          if (d > c) {
            return Atomics.exchange(D, L.Vf >> 2, 0), -73;
          }
    ...
    function Zc(a) {
      const b = a.getExtension("WEBGL_draw_buffers");
      b &&
    ...
    function md(a) {
      ...
      if (!a.oi) {
        ...
        (b.getSupportedExtensions() || []).forEach((d) => {
    ...
    function yd(a, b, c, d, e) {
      ...
      const g = 31 - Math.clz32(a.BYTES_PER_ELEMENT);
      var k = jd;
      return a.subarray(
        e >> g,
        (e +
    ...
    const Jd = ["default", "low-power", "high-performance"];
    let Kd = {};
    function Ld() {
      if (!Md) {
        const a = {
    ...
    function Pd(a, b, c, d) {
      ...
      try {
        a: {
          for (let e = kc(a), g = (a = 0); g < c; g++) {
            const k = D[(b + (8 * g + 4)) >> 2];
            let m = N.read(e, v, D[(b + 8 * g) >> 2], k, void 0);
            if (0 > m) {
              var r = -1;
              break a;
            }
    ...
    function Ud(a, b, c, d) {
      ...
      if (null != g) {
        return (a = e(r, q, t, a, g, k)), (D[d >> 2] = a), 0;
      }
      if (m & 4) {
        return -2;
      }
    ...
    function Yd(a, b) {
      if (!a) {
        return h("pthread_join attempted on a null thread pointer!"), 71;
      }
      if (n && selfThreadId == a) {
    ...
    let be = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
    function ce(a, b) {
      for (a = new Date(a.getTime()); 0 < b; ) {
        const c = a.getMonth();
        var d = (Zd(a.getFullYear()) ? ae : be)[c];
        if (b > d - a.getDate()) {
          (b -= d - a.getDate() + 1),
            a.setDate(1),
            11 > c
    ...
    const ge = new Float32Array(288);
    for (fe = 0; 288 > fe; ++fe) {
      zd[fe] = ge.subarray(0, fe + 1);
    }
    const ke = new Int32Array(288);
    for (fe = 0; 288 > fe; ++fe) {
...
"%W"(p){var x=new Date(p.rf,0,1),z=1===x.getDay()?x:ce(x,0===x.getDay()?1:7-x.getDay()+1);p=new Date(p.rf+1900,p.dg,p.pg);return 0>k(z,p)?g(Math.ceil((31-z.getDate()+($d(Zd(p.getFullYear())?ae:be,p.getMonth()-1)-31)+p.getDate())/7),2):0===k(z,x)?"01":"00"},"%y"(p){return(p.rf+1900).toString().substring(2)},"%Y"(p){return p.rf+1900},"%z"(p){p=p.Ri;var x=0<=p;p=Math.abs(p)/60;return(x?"+":"-")+String("0000"+(p/60*100+p%60)).slice(-4)},"%Z"(p){return p.Ui},
"%%"(){return"%"}};for(t in q){0<=c.indexOf(t)&&(c=c.replace(new RegExp(t,"g"),q[t](d)));}t=$b(c,!1);if(t.length>b){return 0;}v.set(t,a);return t.length-1},x:de,s(a){let b=Date.now()/1E3|0;a&&(D[a>>2]=b);return b}};
    (function () {
      function a(e, g) {
        f.asm = e.exports;
        G = f.asm.Sd;
      ...
      (function () {
        return Ba ||
          ...
          ? c(b)
          : fetch(jb, { credentials: "same-origin" }).then((e) => {
              return WebAssembly.instantiateStreaming(e, d).then(b, (g) => {
    ...
    var Cb = (f._emscripten_main_thread_process_queued_calls = function () {
    ...
    var nb = (f._do_emscripten_dispatch_to_thread = function () {
    ...
    var Hd = (f.__emscripten_call_on_thread = function () {
    ...
    var Ie = (f.dynCall_iiiijj = function () {
